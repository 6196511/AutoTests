...
class Test_GODO982(BaseTest):
    def test_982(self):
        ...
        time.sleep(5)
        page.enter_customer_information_button.click()  # STEP10
        time.sleep(5)
        NewFirstName = 'James'
        page.first_name.send_keys(NewFirstName)  # STEP11
        NewLastName = 'James'
        NewFullName = NewFirstName + ' ' + ''.join(NewLastName)
        page.last_name.send_keys(NewLastName)
        NewEmail = ('James@mailinator.com')
        page.email_address.send_keys(NewEmail)
        time.sleep(10)
        page.complete_booking_button.click()  # STEP12
        time.sleep(5)
        wait = WebDriverWait(get_driver(), 15)
        wait.until(lambda driver: page.is_element_present('payment_type_list'))
        select = Select(page.payment_type_list)  # STEP13
        PaymentType = "Cash"
        select.select_by_visible_text(PaymentType)
        page.submit_booking_button.click()  # STEP14
        time.sleep(5)
        assert page.final_alert.get_attribute("textContent") == 'Booking Successful!'
        page.final_alert_ok_button.click()  # STEP15
