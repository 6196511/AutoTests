...
class Test_GODO326_342(BaseTest):
    def test_326(self):
        ...
        time.sleep(5)
        NewUserName = ("autotestnocomission_" + ''.join(choice(digits) for i in range(4)))
        ...
        ComissionTypes = ('Dollar Amount', 'Percentage')
        NewComissionType = random.choice(ComissionTypes)
    ...
    def test_342(self):
        ...
        page.save_button.click()
        time.sleep(5)
        assert page.channel_exist_alert.get_attribute('textContent') == 'Username (' + ''.join(UserNameList[0]) + ') not valid.'
        page.OK_alert.click()
        time.sleep(5)
