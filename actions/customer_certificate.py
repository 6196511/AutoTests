...
class CustomerCertActions:
    ...
    def open_page(self, certificate):
        self.certificate = CustomerCertPage(driver=self.driver, url=certificate.URL_cert)
    ...
    def verify_payment_info(self, cert):
        ...
        assert self.certificate.booking_fee.text == cert.cert_booking_fee, "Wrong booking fee! '%s'" % \
                                                                      self.certificate.booking_fee.text
        assert self.certificate.total_amount.text == cert.cert_total_amount, "Wrong total amount! '%s'" % \
        ...
        assert self.certificate.full_charge.text == cert.cert_total_amount, "Wrong full charge! '%s'" % \
                                                                       self.certificate.full_charge.text

    def make_successful_payment(self, cert):
        self.certificate.enter_cc_info(cert.cert_card_number, cert.cert_card_date, cert.cert_card_cvc, cert.cert_card_zip)
    ...
    def make_declined_payment(self, cert):
        self.certificate.enter_cc_info(cert.cert_declined_card_number, cert.cert_card_date, cert.cert_card_cvc, cert.cert_card_zip)
        wait(lambda: self.certificate.pay_button.is_enabled())
        self.certificate.pay_button.click()
    ...
    def verify_summary_details(self, cert):
        ...
        assert len(self.certificate.gift_code.text) == 16, "Someting with gift code: '%s'" % self.certificate.gift_code.text
