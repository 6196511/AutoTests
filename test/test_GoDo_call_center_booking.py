...
@pytest.mark.parametrize("tickets", center_data, ids=[repr(x) for x in center_data])
def test_call_center_booking(call_center, tickets):
    """Booking tickets via call center."""
    sleep(1)
    call_center.booking.refresh_page()
...
@pytest.mark.parametrize("tickets", center_declines, ids=[repr(x) for x in center_declines])
...
@pytest.mark.parametrize("tickets", center_valid_codes, ids=[repr(x) for x in center_valid_codes])
...
@pytest.mark.parametrize("tickets", center_invalid_codes, ids=[repr(x) for x in center_invalid_codes])
...
@pytest.mark.parametrize("order", center_booking_with_certificates, ids=[repr(x) for x in center_booking_with_certificates])
